...
# Step 0: Kill existing model server to free GPU memory for training
echo ""
echo ">>> Step 0: Stopping existing model server to free GPU memory..."
echo ""
...
# Step 1: Train
echo ""
echo ">>> Step 1/5: Running QLoRA fine-tuning..."
echo ""
...
echo ""
echo ">>> Step 2/5: Stopping any remaining processes..."
echo ""
...
echo ">>> Step 3/5: Starting model server..."
echo ""

python finetune/serve_model.py \
    --base_model mistralai/Mistral-Small-24B-Instruct-2501 \
    --lora_path "$CHECKPOINT_DIR" \
    --port "$PORT" > /tmp/vllm_server.log 2>&1 &
...
echo ""
echo ">>> Step 4/5: Waiting for model server to be ready..."
echo ""
...
for i in $(seq 1 $TIMEOUT); do
    if ! kill -0 $SERVER_PID 2>/dev/null; then
        ...
        python finetune/serve_model.py \
            ...
            --lora_path "$CHECKPOINT_DIR" \
            --port "$PORT" \
            --merge_and_serve > /tmp/vllm_server.log 2>&1 &
